...
const el = {
  ...
  weightMode: document.querySelector("#weightMode"),
...
function changeQuestionWeight(questionIndex, weight) {
  const question = state.questions[questionIndex];
  if (!question) return;
...
function renderQuestionCard(question, index) {
  ...
  identity.className = "question-identity";

  const weightField = document.createElement("div");
  weightField.className = "question-weight-field";
  weightField.title = `Peso da questão ${question.number}`;
  ...
  const decreaseWeight = document.createElement("button");
  decreaseWeight.type = "button";
  decreaseWeight.className = "weight-step-button";
  decreaseWeight.textContent = "−";
  decreaseWeight.setAttribute(
    "aria-label",
    `Diminuir peso da questão ${question.number}`,
  );
  ...
  weightInput.value = formatWeight(question.weight ?? 1);
  weightInput.setAttribute(
    "aria-label",
    `Peso da questão ${question.number}`,
  );

  const increaseWeight = document.createElement("button");
  increaseWeight.type = "button";
  increaseWeight.className = "weight-step-button";
  increaseWeight.textContent = "+";
  increaseWeight.setAttribute(
    "aria-label",
    `Aumentar peso da questão ${question.number}`,
  );

  const uniformWeight = el.weightMode.value === "uniform";
  decreaseWeight.disabled = uniformWeight;
  weightInput.disabled = uniformWeight;
  increaseWeight.disabled = uniformWeight;

  const commitWeight = rawValue => {
    const weight = normalizeWeight(rawValue);

    question.weight = weight;
    weightInput.value = formatWeight(weight);
    state.currentQuestion = index;
    el.resultsPanel.hidden = true;
  };

  decreaseWeight.addEventListener("click", event => {
    event.stopPropagation();
    commitWeight(Number(question.weight || 1) - 0.1);
  });

  increaseWeight.addEventListener("click", event => {
    event.stopPropagation();
    commitWeight(Number(question.weight || 1) + 0.1);
  });
  ...
  weightStepper.append(decreaseWeight, weightInput, increaseWeight);
  weightField.append(weightText, weightStepper);

  const number = document.createElement("span");
  number.className = "question-number";
  number.textContent = `Questão ${question.number}`;

  identity.append(weightField, number);

  const select = document.createElement("select");
  select.className = "question-options-select";
  select.setAttribute(
    "aria-label",
    `Quantidade de alternativas da questão ${question.number}`,
  );
...
function validateForm() {
  ...
  const totalScore = Number(el.totalScore.value);

  if (!Number.isFinite(totalScore) || totalScore <= 0) {
    return "Informe um valor total positivo para a prova.";
  }

  const invalidWeights = state.questions
    .filter(question => {
      const weight = Number(question.weight);
      return !Number.isFinite(weight) || weight < 0.1 || weight > 1;
    })
    .map(question => question.number);

  if (invalidWeights.length) {
    return (
      `Informe pesos entre 0,1 e 1 nas questões: ` +
      `${invalidWeights.join(", ")}.`
    );
  }
...
async function saveAssessment() {
  ...
  const payload = {
    ...
    total_score: Number(el.totalScore.value),
  ...
  try {
    ...
    el.openAssessment.href = `/avaliacoes/${encodeURIComponent(result.id)}`;
...
el.weightMode.addEventListener("change", () => {
  if (el.weightMode.value === "uniform") {
    state.questions = state.questions.map(question => ({
      ...question,
      weight: 1,
    }));
...
el.totalScore.addEventListener("input", () => {
  el.resultsPanel.hidden = true;
});
